...
std::string format_box(const Box& b) {
  std::ostringstream oss;
  oss << "class=" << b.class_id
      << " score=" << std::fixed << std::setprecision(2) << b.score
...
float box_iou(const ExpectedBox& exp, const Box& pred) {
  return box_iou_xyxy(exp.x1, exp.y1, exp.x2, exp.y2,
...
MatchResult match_expected_boxes(const std::vector<Box>& boxes,
  ...
  for (const auto& b : boxes) {
    if (b.score >= min_score) candidates.push_back(b);
  }
  ...
  for (size_t i = 0; i < expected.size(); ++i) {
    ...
    for (size_t j = 0; j < candidates.size(); ++j) {
      if (used[j]) continue;
      if (candidates[j].class_id != exp.class_id) continue;
    ...
    if (best_idx < 0 || best_iou < min_iou) {
      if (!note.str().empty()) note << ";";
      note << "expected[" << i << "]=" << format_expected(exp)
           << " best_iou=" << std::fixed << std::setprecision(2) << best_iou;
      if (best_idx >= 0) {
...
std::vector<Box> parse_boxes_lenient(const std::vector<uint8_t>& bytes,
                                     ...
                                     int expected_topk) {
  std::vector<Box> out;
  if (bytes.size() < sizeof(uint32_t)) return out;

  uint32_t header = 0;
  std::memcpy(&header, bytes.data(), sizeof(header));
  const size_t payload = bytes.size() - sizeof(header);
  if (payload % 24 != 0) return out;

  const size_t max_boxes = payload / 24;
  if (header > max_boxes) header = static_cast<uint32_t>(max_boxes);
  if (expected_topk > 0) {
    const size_t expected_size = sizeof(header) + static_cast<size_t>(expected_topk) * 24;
    if (bytes.size() < expected_size) return out;
  }
