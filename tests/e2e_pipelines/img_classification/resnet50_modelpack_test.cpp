...
static sima::mpk::ModelMPK mpk_from_format(const std::string& tar_gz,
                                           int width,
                                           int height,
                                           const std::string& format) {
  int depth = 0;
  if (format == "GRAY") depth = 1;
  if (format == "RGB" || format == "BGR") depth = 3;
  return sima::mpk::ModelMPK(tar_gz, "video/x-raw", format, width, height, depth);
}
...
static sima::NeatTensor require_tensor(const sima::RunInputResult& out,
                                       const std::string& label) {
  if (out.kind != sima::RunOutputKind::Tensor || !out.neat.has_value()) {
...
static sima::NeatTensor pull_tensor_with_retry(sima::TensorStream& ts,
                                               const std::string& label,
                                               int per_try_ms,
                                               int tries) {
...
static sima::NeatTensor run_direct_infer(const sima::mpk::ModelMPK& model,
                                         const cv::Mat& rgb,
                                         bool use_debug,
...
static sima::NeatTensor run_image_group_infer(const sima::mpk::ModelMPK& model,
...
static sima::NeatTensor run_rtsp_infer(const sima::mpk::ModelMPK& model,
...
int main(int argc, char** argv) {
  ...
  auto model_rgb = mpk_from_format(tar_gz, kInferWidth, kInferHeight, "RGB");

  try {
    cv::Mat rgb = load_rgb_resized(image_path, kInferWidth, kInferHeight);

    const bool explicit_choice =
        only_direct || only_jpeg || only_rtsp || skip_direct || skip_jpeg;
    bool run_direct = only_direct
        ? true
        : (!only_rtsp && !only_jpeg && !skip_direct);
    bool run_jpeg = only_jpeg
        ? true
        : (!only_rtsp && !only_direct && !skip_jpeg);
    bool run_rtsp = only_rtsp
        ? true
        : (!only_jpeg && !only_direct);

    const int run_count = (run_direct ? 1 : 0) +
                          (run_jpeg ? 1 : 0) +
                          (run_rtsp ? 1 : 0);
    if (!explicit_choice || run_count > 1) {
      std::cout << "[info] MLA pipelines are single-owner per process; "
                   "running direct only. Use --only-jpeg or --only-rtsp "
                   "to test other paths.\n";
      run_direct = true;
      run_jpeg = false;
      run_rtsp = false;
    ...
    if (run_jpeg) {
      auto model_nv12_jpeg = mpk_from_format(
          tar_gz, kJpegInputWidth, kJpegInputHeight, "NV12");
    ...
    if (run_rtsp) {
      auto model_nv12_rtsp = mpk_from_format(
          tar_gz, kRtspInputWidth, kRtspInputHeight, "NV12");
