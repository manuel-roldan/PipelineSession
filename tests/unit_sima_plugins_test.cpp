...
int main() {
  try {
    ...
    {
      sima::SimaBoxDecodeOptions opt;
      opt.config_path = "/tmp/boxdecode.json";
      opt.sima_allocator_type = 2;
      opt.silent = false;
      opt.emit_signals = true;
      opt.transmit = true;

      auto node = sima::nodes::SimaBoxDecode(opt);
      const std::string frag = node->gst_fragment(1);
      require_contains(frag, "simaaiboxdecode name=n1_boxdecode", "boxdecode fragment name mismatch");
      require_contains(frag, "config=\"/tmp/boxdecode.json\"", "boxdecode config missing");
      require_contains(frag, "sima-allocator-type=2", "boxdecode allocator missing");
      require_contains(frag, "silent=false", "boxdecode silent missing");
      require_contains(frag, "emit-signals=true", "boxdecode emit-signals missing");
      require_contains(frag, "transmit=true", "boxdecode transmit missing");

      auto* provider = dynamic_cast<sima::OutputSpecProvider*>(node.get());
      require(provider != nullptr, "boxdecode OutputSpecProvider missing");
      sima::OutputSpec spec = provider->output_spec({});
      require(spec.media_type == "application/vnd.simaai.tensor",
              "boxdecode output media_type mismatch");
      require(spec.format == "BBOX", "boxdecode output format mismatch");
    }
