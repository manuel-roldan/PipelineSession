...
int main(int argc, char** argv) {
  try {
...

    {
      ...
      require(input.tensor->shape.size() >= 2, "debug InputAppSrc tensor missing shape");
      require(input.tensor->shape[0] == 4 && input.tensor->shape[1] == 6,
    ...
    require(out.tensor->shape.size() >= 2, "debug tensor missing shape");
    require(out.tensor->shape[0] == 64 && out.tensor->shape[1] == 64,
    ...
    require(next->tensor->shape.size() >= 2, "debug stream tensor missing shape");
    require(next->tensor->shape[0] == 64 && next->tensor->shape[1] == 64,
